...
(defmacro defsnippet
 ...
 `(def ~(with-meta name {::template source}) (snippet ~source ~selector ~args ~@forms)))
   
(defmacro deftemplate
 "Defines a template as a function that returns a seq of strings." 
 [name source args & forms] 
  `(def ~(with-meta name {::template source}) (template ~source ~args ~@forms)))
 ...
 (let [xml-sym (gensym "xml")]
   `(let [~xml-sym (html-resource ~source)] 
      ~@(for [[name selector args & forms] specs]
               `(def ~(with-meta name {::template source}) (snippet ~xml-sym ~selector ~args ~@forms))))))
